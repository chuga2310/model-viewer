...
import {
  $evictionPolicy,
  CachingGLTFLoader,
} from "./three-components/CachingGLTFLoader.js";
...
export default class ModelViewerElementBase extends ReactiveElement {
  ...
  @property({ type: String }) originUrl: string | null = null;

  /** @export */
  @property({ type: String, attribute: "hashed-content" }) hashedContent:
    | string
    | null = null;
  ...
  constructor() {
    ...
    this[$userInputElement] = shadowRoot.querySelector(
      ".userInput"
    ) as HTMLDivElement;
    this[$canvas] = shadowRoot.querySelector("canvas") as HTMLCanvasElement;
    this[$statusElement] = shadowRoot.querySelector(
      "#status"
    ) as HTMLSpanElement;
  ...
  connectedCallback() {
    ...
    renderer.addEventListener(
      "contextlost",
      this[$onContextLost] as (event: ThreeEvent) => void
    );
